...
import { FOCOS, haversine, USO_COLORS, LINE_COLORS } from '../data/focos'
...
export default function MapView({ selectedId, onSelect, linesVisible, layers }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      gisRef.current.usos = L.geoJSON(usos, {
        style: f => ({
          fillColor: USO_COLORS[f.properties.tipouso] || '#aaa',
          fillOpacity: 0.55,
          color: '#666',
          weight: 0.4,
          opacity: 0.6,
        ...
        onEachFeature: (f, layer) => {
          ...
          layer.bindTooltip(
            `<b>${p.tipouso || '—'}</b><br>${p.sup_ha} ha${p.especie ? ' — ' + p.especie : ''}${p.rodal ? ' — Rodal ' + p.rodal : ''}`,
      ...
      gisRef.current.lineas = L.geoJSON(lineas, {
        style: f => ({
          color: LINE_COLORS[f.properties.tipo] || '#888',
          weight: f.properties.tipo === 'Camino principal' ? 2.5 : 1.5,
          opacity: 0.85,
        }),
        onEachFeature: (f, layer) => {
          const p = f.properties
          layer.bindTooltip(
            `<b>${p.tipo}</b><br>${(p.metros / 1000).toFixed(2)} km`,
            { sticky: true, opacity: 0.95 }
          )
        },
      }).addTo(map)
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      ...
      if (f) {
        map.fitBounds(
          L.latLngBounds([[f.lat1, f.lon1], [f.lat2, f.lon2]]),
          { padding: [100, 100] }
        )
      }
    } else {
